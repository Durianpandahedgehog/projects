...
echo.
echo [4/6] Loading 20 years of historical data...
echo This may take a few minutes...
docker exec corn_app python collect_corn_data.py

echo.
echo [5/6] Training the ensemble model (XGBoost + LSTM)...
echo This will take 5-10 minutes...
docker exec corn_app python ml_model_ensemble.py

echo.
echo [6/6] Generating today's signal...
docker exec corn_app python dashboard.py

echo.
echo Copying dashboard to local folder...
